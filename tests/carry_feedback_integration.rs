...
fn toml_weight_cues_match_rust_defaults() {
    ...
    let toml_content =
        std::fs::read_to_string("assets/config/carry.toml").expect("carry.toml should exist");
    let toml_config: CarryConfig =
        toml::from_str(&toml_content).expect("carry.toml should parse as CarryConfig");
    ...
    assert_eq!(
        toml_cues.footstep_interval_seconds,
        rust_cues.footstep_interval_seconds
    );
    assert_eq!(
        toml_cues.footstep_base_volume,
        rust_cues.footstep_base_volume
    );
    assert_eq!(toml_cues.footstep_max_volume, rust_cues.footstep_max_volume);
    assert_eq!(
        toml_cues.footstep_light_speed,
        rust_cues.footstep_light_speed
    );
    assert_eq!(
        toml_cues.footstep_heavy_speed,
        rust_cues.footstep_heavy_speed
    );
    assert_eq!(toml_cues.bob_base_amplitude, rust_cues.bob_base_amplitude);
    assert_eq!(
        toml_cues.bob_weight_amplitude,
        rust_cues.bob_weight_amplitude
    );
    ...
    assert_eq!(
        toml_cues.breathing_base_speed,
        rust_cues.breathing_base_speed
    );
    assert_eq!(
        toml_cues.breathing_heavy_speed,
        rust_cues.breathing_heavy_speed
    );
    assert_eq!(
        toml_cues.breathing_start_ratio,
        rust_cues.breathing_start_ratio
    );
    assert_eq!(
        toml_cues.breathing_full_ratio,
        rust_cues.breathing_full_ratio
    );
    assert_eq!(
        toml_cues.breathing_max_volume,
        rust_cues.breathing_max_volume
    );
}
...
fn player_input_state_can_be_modified() {
    ...
    assert_eq!(
        action_state.axis_pair(&InputAction::Move),
        Vec2::new(1.0, 0.0)
    );
