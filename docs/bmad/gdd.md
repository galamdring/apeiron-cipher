...
**The found ship as implicit curriculum** -- the starter ship is found mostly intact. It is recognizably a ship. It is recognizably broken. The player does not need to be told to fix it -- the gap between what it is and what it could be is self-evident. Repairing the ship is the first encounter with fabrication: a damaged thruster requires a replacement part, which requires understanding what the part is made of, which requires the material observation and combination systems. The ship teaches crafting by being broken in specific, legible ways. Each repair is a complete loop -- observe the damage, identify the material, fabricate the component, install it -- before the player has encountered any of these systems in abstract. The ship is not a tutorial. It is an object with a history and a problem. The player's response to that problem is the tutorial.

**Ship repair as base building on-ramp** -- hull repairs that require structural panels introduce the same material-to-component-to-structure loop that base building uses. A player who has repaired a ship's hull has already learned base building. The distinction between "fixing a ship" and "building a base" is one of intent, not mechanics. The first time the player constructs a shelter near the ship, the fabrication loop is already familiar. The ship is the tutorial for both systems simultaneously, without being a tutorial for either.
...
**Terrain texture and surface detail** -- terrain is not flat geometry with a color pass. Surface materials have texture that communicates their physical properties: crystalline formations catch light differently than sedimentary rock, soil reads as soft, ice reads as slick. Texture is not cosmetic -- it is the visual language the knowledge-driven rendering system uses to show the player what the ground is made of. A player with geological knowledge should be able to read the terrain surface the way a geologist reads an outcrop. Texture detail is generated from the same material parameters that drive gameplay properties -- the visual and physical are the same data.

**Giant flora and inhabitable structures** -- some biomes produce flora at architectural scale: trees wide enough to build inside, flowers whose open cups form natural bowls large enough to shelter a base, root systems that create tunnels and chambers. These are not decorative -- they are structural elements with interior spaces. A player who chooses to build a base inside a giant flower gets a base whose walls are the flower's interior geometry. The collision boundary follows the visible surface, not a bounding box. If the flower is open at the top, the sky is visible through the opening -- the hitbox traces the petal geometry, not a convex hull around the whole plant. Flora of this scale follows the same rules as all generated structures: every space is functional, scale reflects the ecological investment, and the interior reads coherently (a flower's interior smells, has its own microclimate, attracts specific fauna). Giant flora interiors are valid base locations with their own material palette (biological materials, structural members derived from the plant's own composition) and their own environmental hazards (seasonal opening and closing, fauna that lives there, the plant's own chemical environment).
...
|---------|------|---------|
| 1.1 | 2026-06-07 | Added terrain texture and surface detail; giant flora as inhabitable structures with geometry-following collision; found ship as implicit crafting and base-building curriculum; ship repair as base building on-ramp |
