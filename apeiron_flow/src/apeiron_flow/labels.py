...
from apeiron_flow.config import REPO, REPO_OWNER, REPO_NAME
...
def _remove_label(issue_number: int, label: str) -> None:
    ...
    GitHub returns 200 with the remaining labels on success, and 404 if
    the label was not on the issue. Both are treated as successful removal
    (idempotent).
    """
...
def retire_in_review(issue_number: int) -> None:
    ...
    This is a one-time migration helper to retire the legacy status:in-review
    label in favour of the two new labels introduced by ADR 002. Only the
    human-review semantics are preserved here (the issue is passed agent-review
