...
class TestTransition:
    ...
    def test_happy_path_adds_then_removes(self, mock_get, mock_post):
        ...
        # _gh_post for adding, _gh_delete will be called separately
        ...
        with patch("apeiron_flow.labels._gh_delete") as mock_delete:
            ...
            mock_delete.assert_called_once_with(
                f"/repos/{owner}/{repo}/issues/42/labels/status%3Atriage"
                if False  # URL encoding depends on requests, see below
                else f"/repos/{owner}/{repo}/issues/42/labels/status:triage"
            )
