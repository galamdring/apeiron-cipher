...
fn all_three_biomes_reachable() {
    ...
    for x in -50..50 {
        for z in -50..50 {
            ...
            found.insert(biome.biome_type.clone());
            if found.len() == 3 {
                break;
            }
        }
...
fn fallback_biome_used_when_no_range_matches() {
    ...
    let registry = BiomeRegistry {
        ...
        fallback_biome_type: "mineral_steppe".to_string(),
        biomes: vec![
    ...
    for x in 0..20 {
        ...
        if biome.biome_type == "mineral_steppe" {
            found_fallback = true;
            assert_eq!(
...
fn empty_registry_returns_hardcoded_neutral_default() {
    ...
    let registry = BiomeRegistry {
        biomes: vec![],
        fallback_biome_type: "frost_shelf".to_string(),
    ...
    assert_eq!(result.biome_type, "frost_shelf");
...
fn chunk_biome_includes_correct_palette_for_each_biome_type() {
    ...
    for x in -50..50 {
        for z in -50..50 {
            ...
            verified.insert(biome.biome_type.clone());
...
fn planet_env_none_uses_normalized_matching_only() {
    ...
    for x in 0..20 {
        let biome = derive_chunk_biome(&profile, &registry, ChunkCoord::new(x, 0), None);
        assert!(
            biome.biome_type == "scorched_flats" || biome.biome_type == "frost_shelf",
            "unexpected biome: {}",
            biome.biome_type,
        );
    }
...
fn planet_env_cold_planet_filters_hot_biome() {
    ...
    for x in 0..50 {
        ...
        assert_ne!(
            biome.biome_type, "scorched_flats",
...
fn hot_planet_biomes_differ_from_cold_planet_biomes() {
    ...
    assert!(
        !hot_biomes.contains(&"frost_shelf".to_string()),
        "hot planet should not contain FrostShelf",
    );
    assert!(
        !cold_biomes.contains(&"scorched_flats".to_string()),
...
fn override_mode_biome_generation_no_regression() {
    ...
    for x in -50..50_i32 {
        for z in -50..50_i32 {
            ...
            found_biomes.insert(biome_a.biome_type.clone());
        }
    }
